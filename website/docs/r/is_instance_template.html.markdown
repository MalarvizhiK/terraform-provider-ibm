...
description: |-
  Manages IBM IS Instance Template.
---

# ibm\_is_instance_template

Provides a instance template resource. This allows instance template to be created, updated, and cancelled.


## Example Usage

Here is an example of instance template with 'Public' virtual server type.
...
Here is an example of instance template with 'Dedicated' virtual server type. 
...
resource "ibm_is_instance_template" "testacc_instance_template" {
  ...
  placement_target {
        ...
        id = ibm_is_dedicated_host_group.test_dedicated_host_group.id
  }
...
* `name` - (Optional, string) The instance template name.
* `vpc` - (Required, Forces new resource, string) The vpc id. 
* `zone` - (Required, Forces new resource, string) Name of the zone. 
* `profile` - (Required, Forces new resource, string) The profile name. 
* `image` - (Required, string) ID of the image.
* `boot_volume` - (Optional, list) A block describing the boot volume of the instance.  
...
* `primary_network_interface` - (Required, list) A nested block describing the primary network interface of the instance. We can have only one primary network interface.
...
* `network_interfaces` - (Optional, Forces new resource, list) A nested block describing the additional network interface of the instance.
Nested `network_interfaces` block have the following structure:
  * `name` - (Optional, string) The name of the network interface.
  * `subnet` -  (Required, string) ID of the subnet.
  * `security_groups` - (Optional, list) Comma separated IDs of security groups.
* `placement_target` - (Required, list) A nested block describing the placement type of the instance template. We can have only one primary network interface.
Nested `placement_target` block have the following structure:
  * `resource_type` - (Required, string) The resource type of value dedicated_host or dedicated_host_group
  * `id` - (Required, string) The ID of dedicated host or dedicated host group.
...
* `resource_group` - (Optional, Forces new resource, string) The resource group ID for the instance.
...
Nested `network_interfaces` blocks have the following structure:
  ...
  * `security_groups` -  List of security groups.
  * `primary_ipv4_address` - The primary IPv4 address.
* `placement_target` - A nested block describing the placement type of the instance template.
Nested `placement_target` block have the following structure:
  * `id` - The ID of dedicated host or dedicated host group.
...
* `resource_controller_url` - The URL of the IBM Cloud dashboard that can be used to explore and view details about the instance.


## Import

ibm_is_instance_template can be imported using instanceID, eg

```
$ terraform import ibm_is_instance_template.example d7bec597-4726-451f-8a63-e62e6f19c32c
```
